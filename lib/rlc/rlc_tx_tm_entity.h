...
/// \brief Stub TX TM entity.
class rlc_tx_tm_entity : public rlc_tx_entity
{
public:
  rlc_tx_tm_entity(gnb_du_id_t          du_index_,
    ...
    cfg(config)
  {
    metrics.tx_low.mode_specific = rlc_tm_tx_metrics_lower{};
  }

  void handle_sdu(byte_buffer sdu_buf, bool is_retx) override { logger.warning("TX TM SDU not implemented"); }
  void discard_sdu(uint32_t pdcp_sn) override { logger.warning("TX TM discard not implemented"); }
