...
size_t rlc_um_write_data_pdu_header(uint8_t* buffer, const rlc_um_pdu_header& header) {
    if (!buffer) return 0;
...
size_t rlc_um_write_data_pdu_header(byte_buffer& buffer, const rlc_um_pdu_header& header) {
...
bool rlc_um_read_data_pdu_header(const uint8_t* buffer, rlc_um_sn_size sn_size, rlc_um_pdu_header* header) {
    // This wrapper is kept for API compatibility but is unsafe without size.
    // The byte_buffer version is preferred.
    ...
    if (sn_size == rlc_um_sn_size::size6bits) {
        ...
        header->so = (header->si != rlc_si_field::first_segment) ? ((buffer[1] << 8) | buffer[2]) : 0;
    } else { // 12-bit
        header->sn = ((buffer[0] & 0x0F) << 8) | buffer[1];
        header->so = (header->si != rlc_si_field::first_segment) ? ((buffer[2] << 8) | buffer[3]) : 0;
    }
    return true;
}

bool rlc_um_read_data_pdu_header(const byte_buffer& buffer, rlc_um_sn_size sn_size, rlc_um_pdu_header* header) {
    if (!header || buffer.length() == 0) return false;
    
    const uint8_t* data = buffer.get_span().data();
    size_t size = buffer.length();
    
    rlc_um_pdu_header temp_header;
    temp_header.si = static_cast<rlc_si_field>((data[0] >> 6) & 0x03);
    temp_header.sn_size = static_cast<uint32_t>(sn_size);
    
    if (temp_header.si == rlc_si_field::full_sdu) {
        temp_header.sn = INVALID_RLC_SN;
        temp_header.so = 0;
        *header = temp_header;
        return true;
    }
    ...
    if (sn_size == rlc_um_sn_size::size6bits) {
        ...
        temp_header.sn = data[0] & 0x3F;
        if (temp_header.si != rlc_si_field::first_segment) {
            if (size < 3) return false;
            temp_header.so = (data[1] << 8) | data[2];
        } else {
            temp_header.so = 0;
        }
    } else { // 12-bit
        if (size < 2) return false;
        temp_header.sn = ((data[0] & 0x0F) << 8) | data[1];
        if (temp_header.si != rlc_si_field::first_segment) {
            if (size < 4) return false;
            temp_header.so = (data[2] << 8) | data[3];
        } else {
            temp_header.so = 0;
        }
    }
